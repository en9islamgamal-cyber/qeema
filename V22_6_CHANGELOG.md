...
## v22.6.3 — CORRECTED (re-issued after honest audit)

### What was wrong with the first cut of v22.6.3

After the first v22.6.3 was shipped, an honest audit revealed three
silent failure modes:

1. **Fix 3 + Fix 5 wrote to the wrong storage system.** QEEMA has
   *two* phase state systems:
   - `core/phase_state.py` (PhaseStateManager): writes to
     `state/phases/episode_NNN.json` and IS persisted across workflow
     runs via the GitHub Actions cache + artifact upload.
   - `orchestrator._save_phase_state` / `_load_phase_state`: writes to
     `temp/episodes/episode_NNN/_phase_state.json` and is **ephemeral**
     between runs.

   The first v22.6.3 wrote `_deep_visuals`, `_tts_directions`, and
   `episode_json_snapshot` to the orchestrator's ephemeral system. Within
   a single run that worked, but Phase 3 running on a fresh runner saw
   none of those values — defeating the entire point of the fix.

2. **Fix 4 had no test.** The `refreshed = self._reload_episode_script`
   call after deep visuals was added to the main loop, but no test
   verified the wiring. A regression could silently break it.

3. **The unit tests gave false confidence.** All v22.6.3 unit tests
   used `MagicMock(spec=Orchestrator)` and stubbed every method on the
   surface — exactly mirroring the broken implementation. The tests
   would have passed even if every fix wrote nothing to disk.

### What changed in the corrected v22.6.3

**Fix 3 — `phase_router._run_phase_2` propagates Phase 2 outputs**
The orchestrator still writes `_deep_visuals` and `_tts_directions` into
the temp episode JSON during Phase 2 (that's where downstream synthesis
reads them within the same run). After the orchestrator returns,
`phase_router._run_phase_2` reads them from the temp JSON and includes
them in the `asset_paths` dict it returns to `mark_phase_complete`.
PhaseStateManager then persists everything to `state/phases/`.

**Fix 5 — `_reload_episode_script` uses PERSISTENT state**
Removed the `_save_phase_state(episode_json_snapshot=...)` call from
Phase 1 close (it wrote to the wrong system). Replaced with a read from
`PhaseStateManager.load(episode_number).script_data` — which IS what
gets saved by `phase_router._run_phase_1`'s output of
`{"script_data": ...}`. This is a real fix, not a layer.

**Fix 4 — Static source-inspect test added**
A test now uses `inspect.getsource(Orchestrator._run_pipeline)` and
verifies textually that:
1. `_run_phase2_deep_visuals` is called
2. `refreshed = self._reload_episode_script` appears AFTER (1)
3. The AI-images stage appears AFTER (2)

This won't catch every bug but it locks the ordering invariant against
accidental refactor.

**Phase 3 hydration**
The Phase 3 standalone branch now uses `PhaseStateManager` directly
(not the orchestrator's ephemeral state) and reconstructs the temp
episode JSON from `state.script_data + state.asset_paths._deep_visuals
+ state.asset_paths._tts_directions` BEFORE calling
`_reload_episode_script`. Without this, the script object Phase 3 builds
would have empty `visual_prompt` fields and missing SSML — the rendered
video would be CSS-fallback boxes with bare text-to-speech.

**`PhaseState.asset_paths` type annotation widened**
Was `Dict[str, str]` — now `Dict[str, Any]` because it now holds nested
lists/dicts (`_deep_visuals` is a list of dicts, `_tts_directions` is
a dict of dicts). JSON survives the round-trip regardless, but the
annotation reflects reality.

### Test coverage (corrected)

Two new files cover the corrected approach with tests that exercise
real wiring rather than mock surfaces:

- `tests/test_v22_6_3_phase2_fixes.py` — 15 tests covering Fix 1-5
  individually, including:
  - Phase router's actual `_run_phase_2` reads the temp JSON
    (uses `PhaseRouter.__new__` to skip `__init__`, then fakes the
    orchestrator).
  - `_reload_episode_script` patches `core.phase_state.PhaseStateManager`
    and verifies the temp JSON is restored from `state.script_data`.
  - Static source-inspect tests for ordering invariants in
    `_run_pipeline`.

- `tests/test_v22_6_3_e2e_persistence.py` — 2 end-to-end tests that
  instantiate a real `PhaseStateManager` against a tmp directory,
  simulate three workflow runs (Phase 1 → save → wipe → load → Phase
  2 → save → wipe → load → Phase 3), and verify everything that needs
  to survive does.

Total suite: **574 tests**, all pass with and without `google-genai`.

### Files changed in corrected v22.6.3

```
orchestrator.py                       — Fix 5 reads PhaseStateManager;
                                        Phase 3 hydration; wrong-system
                                        writes removed
core/phase_router.py                  — Fix 3 propagates Phase 2 outputs
core/phase_state.py                   — asset_paths type widened
tests/test_v22_6_3_phase2_fixes.py    — rewritten for corrected approach
tests/test_v22_6_3_e2e_persistence.py — NEW: full three-run simulation
V22_6_CHANGELOG.md                    — this section
```

### What is still NOT verified

- **Real Gemini call against the simplified `BatchTTSOut` schema.**
  Removing the bounds is conservative but only a real production run
  will confirm Gemini accepts the simplified schema.
- **Actual GitHub Actions cache behaviour with `state/phases/`.** The
  workflow YAML already caches this directory (we didn't change that),
  but a real cross-day run is the only way to confirm cache hits.
- **The Phase 3 ai_images path.** Phase 3 hydration restores
  `_deep_visuals` to the temp JSON, but if Phase 2 didn't generate
  Leonardo images successfully (e.g., quota exhausted), the rendered
  video will still use CSS fallback. v22.6.3 fixes the data-loss
  problem; it doesn't make Leonardo more reliable.
