...
class Orchestrator:
    ...
    def _build_phase2_adapters_dict(
        self, primary_adapter: Any,
    ) -> Dict[str, Any]:
        """v22.6.1: Build adapters dict for Phase 2 batch engines.

        Phase 2 priority is Key 2 → 3 → 1, so we need all 3 adapters
        available for rotation. Reads keys directly from env.
        """
        from infrastructure.llm_adapters import GeminiJsonAdapter
        adapters: Dict[str, Any] = {}

        # Primary (key 2) — already built
        if primary_adapter is not None:
            adapters["gemini-2"] = primary_adapter

        # Build adapter for key 3 if available
        key3 = os.getenv("GEMINI_API_KEY_3", "")
        if key3:
            try:
                adapters["gemini-3"] = GeminiJsonAdapter(
                    key3, model="gemini-2.5-flash",
                    instance_name="phase2-gemini-3",
                )
            except Exception as e:
                logger.warning(f"⚠️ Phase 2: gemini-3 init failed: {e}")

        # Build adapter for key 1 (last resort)
        key1 = os.getenv("GEMINI_API_KEY", "")
        if key1:
            try:
                adapters["gemini-1"] = GeminiJsonAdapter(
                    key1, model="gemini-2.5-flash",
                    instance_name="phase2-gemini-1",
                )
            except Exception as e:
                logger.warning(f"⚠️ Phase 2: gemini-1 init failed: {e}")
    ...
    def _run_phase2_deep_visuals(
        ...
        """v22.6.1: Generate visual prompts using Phase 2 Gemini quota.

        First tries BatchVisualPromptEngine (1 Gemini call for all 7 ayahs).
        Falls back to DeepVisualPromptGenerator (3 layers × 7 = 21 calls)
        if batch fails.
        """
        import json
        ep_path = (
        ...
        try:
            ...
            logger.warning(f"⚠️ Phase 2 deep visuals: cannot read JSON: {e}")
            return

        # v22.6.1: Try batch path first (1 Gemini call instead of 21)
        try:
            adapters_dict = self._build_phase2_adapters_dict(adapter)
            from engines.batch_engines import BatchVisualPromptEngine
            batch_engine = BatchVisualPromptEngine(adapters_dict)

            ayah_scripts = [
                {
                    "number": s.get("ayah", {}).get("number", 0),
                    "explain": s.get("explain_text", ""),
                    "story": s.get("story_text", "") or "",
                }
                for s in episode_data.get("ayah_scenes", [])
            ]

            batch_result = batch_engine.generate_visuals(ayah_scripts)
            if batch_result is not None:
                # Convert BatchVisualOut → _deep_visuals format
                episode_data["_deep_visuals"] = [
                    {
                        "subject": p.subject,
                        "environment": p.environment,
                        "mood": p.mood_lighting,
                        "color_palette": p.color_palette,
                        "is_usable": True,
                        # Fields not in batch output (defaults for compat)
                        "action": "", "time_of_day": "",
                        "lighting_direction": "",
                        "atmospheric_elements": "",
                        "camera_angle": "", "depth_of_field": "",
                        "foreground": "", "midground": "", "background": "",
                        "focal_point": "", "layers_completed": 1,
                    }
                    for p in batch_result.prompts
                ]
                with open(ep_path, "w", encoding="utf-8") as f:
                    json.dump(episode_data, f, ensure_ascii=False, indent=2)
                logger.info(
                    f"🎨 Phase 2 batch visuals: {len(batch_result.prompts)} "
                    f"prompts in 1 call (saved {len(batch_result.prompts)*3 - 1} calls)"
                )
                return
        except Exception as e:
            logger.warning(
                f"⚠️ Batch visual prompts failed ({type(e).__name__}: {e}) — "
                f"falling back to legacy DeepVisualPromptGenerator"
            )

        # Fallback: legacy 21-call path
        try:
            ...
            fully = sum(1 for r in deep_results if getattr(r, 'is_complete', False))
            logger.info(
                f"🎨 Phase 2 deep visuals (legacy): {successful}/{len(deep_results)} "
                f"usable, {fully} fully completed (3 layers)"
            )
        except Exception as e:
            logger.warning(
    ...
    def _run_phase2_tts_director(
        ...
        """v22.6.1: Generate TTS direction using Phase 2 Gemini quota.

        Tries BatchTTSDirector first (1 call instead of 7).
        Falls back to legacy TTSDirector on failure.
        """
        import json
        ep_path = (
            self.paths.temp_episodes / f"episode_{episode_number:03d}.json"
        ...
        try:
            ...
            logger.warning(f"⚠️ Phase 2 TTS director: cannot read JSON: {e}")
            return

        # v22.6.1: Try batch path first (1 call instead of 7)
        try:
            adapters_dict = self._build_phase2_adapters_dict(adapter)
            from engines.batch_engines import BatchTTSDirector
            batch_director = BatchTTSDirector(adapters_dict)

            ayah_scripts = [
                {
                    "number": s.get("ayah", {}).get("number", 0),
                    "emotion": s.get("scene_emotion", "warm"),
                    "text": s.get("explain_text", "")[:200],
                }
                for s in episode_data.get("ayah_scenes", [])
            ]

            batch_result = batch_director.direct_episode(ayah_scripts)
            if batch_result is not None:
                # Convert BatchTTSOut → _tts_directions format
                episode_data["_tts_directions"] = {
                    f"ayah_{d.ayah_number}": {
                        "voice_speed": d.voice_speed,
                        "voice_stability": d.voice_stability,
                        "voice_style": d.voice_style,
                        "emphasis_words": list(d.emphasis_words),
                    }
                    for d in batch_result.directions
                }
                with open(ep_path, "w", encoding="utf-8") as f:
                    json.dump(episode_data, f, ensure_ascii=False, indent=2)
                logger.info(
                    f"🎙️ Phase 2 batch TTS: {len(batch_result.directions)} "
                    f"directions in 1 call (saved {len(batch_result.directions)-1} calls)"
                )
                return
        except Exception as e:
            logger.warning(
                f"⚠️ Batch TTS director failed ({type(e).__name__}: {e}) — "
                f"falling back to legacy TTSDirector"
            )

        # Fallback: legacy per-segment path
        try:
            ...
            if episode_direction.segments:
                episode_data["_tts_directions"] = {
                    ...
                    for sd in episode_direction.segments.values()
                }
                with open(ep_path, "w", encoding="utf-8") as f:
                    json.dump(episode_data, f, ensure_ascii=False, indent=2)
                logger.info(
                    f"🎙️ Phase 2 TTS director (legacy): "
                    f"{len(episode_direction.segments)} segments directed"
                )
            else:
                logger.info(
    ...
    def _try_batch_tafsir(
        ...
        """Try batch tafsir review with multi-key rotation.

        Returns None on any failure (caller falls back to per-ayah).
        """
        # Get the Gemini reviewer (which has all 3 clients + keys)
        reviewer = getattr(self.tafsir_validator, "_gemini_reviewer", None)
        if reviewer is None:
            return None

        # Build adapter-like dict from reviewer's clients for batch_engines
        # _clients is {api_key: client}, _keys is [api_key, ...]
        clients_by_key = getattr(reviewer, "_clients", {})
        keys_list = getattr(reviewer, "_keys", [])
        if not clients_by_key or not keys_list:
            return None

        # Map to "gemini-1", "gemini-2", "gemini-3" naming
        # batch_engines needs adapter._client and adapter._rate_limiter
        from core.gemini_rate_limiter import limiter_for_key

        class _AdapterProxy:
            def __init__(self, c, k):
                self._client = c
                # Bind rate limiter to this key so batch respects RPM limits
                self._rate_limiter = limiter_for_key(
                    k, label_hint="tafsir-batch"
                )

        adapters_proxy: Dict[str, Any] = {}
        for i, api_key in enumerate(keys_list[:3], start=1):
            client = clients_by_key.get(api_key)
            if client is None:
                continue
            adapters_proxy[f"gemini-{i}"] = _AdapterProxy(client, api_key)

        if not adapters_proxy:
            return None
        ...
        # Run batch review with multi-key rotation
        from engines.batch_engines import BatchTafsirReviewer
        engine = BatchTafsirReviewer(adapters_proxy)

        logger.info(
            "🔍 Tafsir validation (BATCH v22.6.1 — 1 Gemini call, "
            f"{len(adapters_proxy)} keys for rotation)"
        )
        ...
        for review in result.reviews:
            ...
            effectively_passed = review.passed and review.confidence >= threshold
            if effectively_passed:
                ...
                for i, c in enumerate(review.concerns, 1):
