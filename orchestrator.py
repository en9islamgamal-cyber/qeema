"""
orchestrator.py — VALUE / QEEMA v15.0 (Cinematic + Quality + Cost)
==========================================================
[Pipeline Stages v15 — in order]
...
# ════════════════════════════════════════════════════════════════
class Orchestrator:
    """Production orchestrator with full DI — v15 cinematic pipeline."""
    ...
    def __init__(
        ...
        *,
        ...
        image_engine=None,  # v16: Optional[LeonardoImageEngine]
        dry_run: bool = False,
        ...
        self.image_engine = image_engine  # v16

        # v15 engines
        self.bgm_mixer = bgm_mixer or BGMMixer(paths=paths)
        self.subtitle_engine = subtitle_engine or SubtitleEngine(paths=paths)

        # Feature flags — v15: all readable from ENV for runtime control
        self.enable_subtitles = enable_subtitles or (
            os.getenv("ENABLE_SUBTITLES", "false").lower() == "true"
        )
        # Allow ENV override (default True from constructor unless ENV says "false")
    ...
    def _run_pipeline(self, *, episode_number, report, start, log, idem_key) -> EpisodeRunReport:
        ...
        try:
            ...
            # ── Stage 7: Color grade (v14 NEW)
            if self.enable_color_grade:
                graded_video = ep_dir / "graded_episode.mp4"
                graded_result = self._run_stage("color_grade",
                    lambda: self.bgm_mixer.apply_color_grade(post_subs, str(graded_video)),
                    report)
                pre_brand = graded_result if Path(graded_result).exists() else post_subs
            else:
                pre_brand = post_subs

            # ── Stage 8: Wrap branded (intro + outro)
            # v15: Pass CTA audio path so outro plays subscribe voice
            ...
            if f"{sid}_ayah" in audio_map:
                ...
                self.visual_renderer.render(SceneRenderRequest(
                    ...
                    extra={
                        ...
                        # No background_image for ayah recitation — clean CSS
                    },
