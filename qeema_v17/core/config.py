"""
core/config.py — VALUE / QEEMA v17.0 (Revolution)
=====================================================
[v17 changes]
- ImageGenConfig.width = 1536 (was 1920 → exceeded Leonardo API limit)
- ImageGenConfig.height = 864 (16:9 ratio at allowed dimensions)
- ProceduralConfig.browser_pool_size = 3 (was 1 → render time -66%)
- EngineConfig.voice_parallel_workers = 3 (was 4 → ElevenLabs Starter+ concurrent limit)
- VideoConfig.color_grade_vf: inline color grade filter (replaces global pass)
- New constants: STAGE_TIMEOUTS for per-stage limits

[v16 changes kept]
- ImageGenConfig (Leonardo paid)
- AudioConfig.elevenlabs_adaptive
- BrandingConfig.channel_tagline_ar (no grandfather)
- AppConfig.image_gen
"""
...
class VideoConfig:
    ...
    # v17: inline color grade — applied during per-scene encoding (no separate stage)
    # eq filter: brightness + saturation; colorchannelmixer: warm tint
    # Set to None to disable color grade entirely.
    color_grade_vf: Optional[str] = (
        "eq=brightness=0.020:saturation=1.08,"
...
class ImageGenConfig:
    """v17 — Leonardo.ai paid Apprentice plan, dimensions corrected for API."""
    enabled: bool = True
    # Model UUIDs
    ...
    # v17 FIX: Leonardo API max dimension is 1536, not 1920
    # 1536x864 keeps 16:9 ratio; HTML scales to 1920x1080 via background-size:cover
    width: int = 1536
    height: int = 864
    # Quality
    enable_alchemy: bool = False
...
class EngineConfig:
    ...
    # v17: 4 → 3. ElevenLabs Starter+ concurrent limit is 4; 3 leaves headroom
    # to avoid 429 on the first call.
    voice_parallel_workers: int = 3
    voice_enable_cache: bool = True
    # v17: stage-level timeout (overall budget for render_scenes etc.)
