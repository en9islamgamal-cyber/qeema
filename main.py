"""
main.py — VALUE / QEEMA v16.0 (Production)
==================================================
Composition root — wires everything together.

[New in v16.0]
  - REMOVED: "جدو أبو زياد" character entirely
  - REMOVED: Storytelling-with-named-children pattern
  - ADDED: Hook-first / insight-first methodology (TED-Ed style)
  - ADDED: Leonardo.ai image generation (paid plan)
  - ADDED: Adaptive ElevenLabs voice (per-emotion overrides)
  - ADDED: Safe SSML extraction (keep <break> tags only)
  - UPDATED: Quality scoring rejects clichés ("يا أحبائي", "كان يا ما كان")

[v15 features kept]
  - CTA voice synthesis
  - Parallel ayah generation across Gemini keys
  - Quality gate with auto-retry
  - Local font + PNG logo embedding
  - Configurable cinematic features via ENV
"""
...
def _build_orchestrator(
    ...
    if _HAS_V15_QUALITY:
        ...
        logging.getLogger("main").info(
            f"✅ Using v15 quality scorer (threshold={threshold})"
        )
    ...
    # ── v15: Cinematic engines (BGM mixer always; subtitle conditional)
    bgm_mixer = BGMMixer(
        paths=config.paths,
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            registry.dump_to_file(metrics_path)
