"""
main.py — VALUE / QEEMA v21.0 (UNIFIED PRODUCTION)
==================================================
Composition root — wires everything together.

[v21 — Full Wiring Revolution]
This version actually CONNECTS all the v18-v20 features that were dead code:
  ✓ Multi-task script generation (v20 — single Gemini call)
  ✓ Batched tafsir validation (v20 — single Claude call)
  ✓ Combined per-scene TTS (v20)
  ✓ Quota-aware degradation strategy (v21 — auto-select mode)
  ✓ Cost dashboard (v20 — generated each run)
  ✓ Pipeline strategy decision-maker (v21 NEW)
  ✓ Local file repository (v20.1)
...
[v20.1 fixes preserved]
  - LocalRepository for testing (--skip-supabase flag)
  - TafsirValidator init signature fallback
  - SKIP_SUPABASE env var support
"""
...
# ════════════════════════════════════════════════════════════════
# TafsirValidator builder — handles signature variants gracefully
# ════════════════════════════════════════════════════════════════
def _build_tafsir_validator(
    anthropic_key: Optional[str],
    gemini_review_key: Optional[str] = None,
) -> Any:
    """Build a TafsirValidator handling multiple constructor signatures.

    v22.5: Accepts both anthropic_key (Claude) and gemini_review_key
    (Gemini fallback). Builds the validator if EITHER is provided —
    Gemini-only operation is now supported.

    Returns None if both keys are missing or construction fails.
    """
    if not _HAS_TAFSIR_VALIDATOR:
        return None

    # v22.5: At least one reviewer key must be provided
    if not anthropic_key and not gemini_review_key:
        return None

    log = logging.getLogger("main")

    # Try newest signature first (v22.5 with gemini_review_key support)
    attempts = [
        # v22.5: both keys
        lambda: TafsirValidator(
            anthropic_key=anthropic_key,
            gemini_review_key=gemini_review_key,
        ),
        # v21 attempt: full kwargs (no gemini)
        lambda: TafsirValidator(
            anthropic_key=anthropic_key,
            review_model="claude-opus-4-7",
        ),
        # v20 attempt: just anthropic_key
        lambda: TafsirValidator(anthropic_key=anthropic_key),
        # v19 attempt: positional
        lambda: TafsirValidator(anthropic_key),
    ]
    last_err: Optional[Exception] = None
    for attempt in attempts:
        try:
            v = attempt()
            if anthropic_key and gemini_review_key:
                log.info(
                    "✅ TafsirValidator wired (Claude + Gemini fallback)"
                )
            elif anthropic_key:
                log.info("✅ TafsirValidator wired (Claude only)")
            else:
                log.info("✅ TafsirValidator wired (Gemini only — Claude unavailable)")
            return v
        except TypeError as e:
            last_err = e
            continue
        except Exception as e:
            log.error(f"❌ TafsirValidator unrecoverable error: {e}")
            return None

    log.error(
        f"❌ TafsirValidator init failed (no compatible signature): {last_err}"
    )
    return None
...
def _build_orchestrator(
    ...
    if not tafsir_validator and not config.api_keys.anthropic:
        log.warning(
