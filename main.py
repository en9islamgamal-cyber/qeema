"""
main.py — VALUE / QEEMA v18.0 (PRODUCTION REVOLUTION)
==================================================
Composition root — wires everything together.

[v18 — Quality + Safety Revolution]
  - CRITICAL: TafsirValidator (religious accuracy via Claude Opus)
  - VisualPromptEngineer (locked style — no visual drift)
  - Re-tuned voice settings for child engagement
  - Per-emotion color grading (5 emotional grades)
  - Word-level subtitle timestamps from ElevenLabs
  - HookOptimizer (Thompson Sampling bandit)
  - 3 thumbnail variants (YouTube Test & Compare)
  - ReviewGate for first 10 episodes (Telegram + manual)
  - Cost tracker integration
  - Subtitles enabled by default

[v17 fixes kept]
  - Color grade baked into per-scene encoding
  - Stream-copy wrap_branded (5s vs 15min)
  - Leonardo dimensions 1536x864 (API limit)
  - Browser pool size 3 (parallel)
"""
...
# v19: quota manager (CRITICAL for fixed-budget operation)
try:
...
def _build_orchestrator(
    ...
    # v19: Initialize QuotaManager FIRST so all engines can use it
    quota_manager = None
    if _HAS_QUOTA_MANAGER:
        try:
            # Use QuotaConfig from app config if available
            ...
            logging.getLogger("main").warning(f"⚠️ QuotaManager init failed: {e}")
    ...
    voice_engine = VoiceEngine(
        ...
        engine_cfg=config.engine,
        quota_manager=quota_manager,  # v19
    )
    ...
    if _HAS_IMAGE_ENGINE and config.api_keys.leonardo and config.engine.enable_ai_images:
        try:
            ...
            image_engine = LeonardoImageEngine(leo_cfg, quota_manager=quota_manager)
    ...
    # ── v18: TafsirValidator (CRITICAL — religious accuracy)
    tafsir_validator = None
    if _HAS_TAFSIR_VALIDATOR and config.api_keys.anthropic:
        try:
            tafsir_validator = TafsirValidator(
                anthropic_key=config.api_keys.anthropic,
                review_model="claude-opus-4-7",
            )
            logging.getLogger("main").info(
                "✅ TafsirValidator wired (Claude Opus religious review)"
            )
        except Exception as e:
            logging.getLogger("main").error(
                f"❌ TafsirValidator init failed: {e}"
            )
            tafsir_validator = None
    elif not config.api_keys.anthropic:
        logging.getLogger("main").warning(
            "⚠️ ANTHROPIC_API_KEY not set — religious validation disabled "
            "(NOT RECOMMENDED for production)"
        )

    # ── v18: HookOptimizer (data-driven hook selection)
    hook_optimizer = None
    if _HAS_HOOK_OPTIMIZER:
        try:
            hook_optimizer = HookOptimizer(paths=config.paths)
            logging.getLogger("main").info("✅ HookOptimizer wired")
        except Exception as e:
            logging.getLogger("main").warning(f"⚠️ HookOptimizer init failed: {e}")

    # ── v18: ReviewGate (manual review for first N episodes)
    review_gate = None
    if _HAS_REVIEW_GATE:
        try:
            ...
            logging.getLogger("main").info(
                f"✅ ReviewGate wired (threshold={review_threshold})"
            )
        except Exception as e:
            logging.getLogger("main").warning(f"⚠️ ReviewGate init failed: {e}")
    ...
    if _HAS_COST_TRACKER:
        try:
            ...
            logging.getLogger("main").info("✅ CostTracker wired")
        except Exception as e:
            logging.getLogger("main").warning(f"⚠️ CostTracker init failed: {e}")

    # ── v18: Per-emotion color grades (from config)
    from core.config import COLOR_GRADES_BY_EMOTION
    color_grades = COLOR_GRADES_BY_EMOTION if enable_color_grade else {}

    # ── Orchestrator (v18: wire all production-grade features)
    return Orchestrator(
