...
# ════════════════════════════════════════════════════════════════
def _build_tafsir_validator(anthropic_key: Optional[str]) -> Any:
    """Build a TafsirValidator handling multiple constructor signatures.

    Returns None if construction fails or key missing.
    Tries newest signature first, falls back to older variants.
    """
    if not _HAS_TAFSIR_VALIDATOR or not anthropic_key:
        return None

    log = logging.getLogger("main")

    # Try newest signature (v21 with review_model + cache_dir)
    attempts = [
        # v21 attempt: full kwargs
    ...
    for attempt in attempts:
        try:
            v = attempt()
            log.info("✅ TafsirValidator wired (Claude Opus religious review)")
            return v
...
def _build_orchestrator(
    ...
    tafsir_validator = _build_tafsir_validator(config.api_keys.anthropic)
