...
class TestPhaseKeyIsolation:
    ...
    def test_phase2_uses_key_2_via_env_lookup(self):
        """Phase 2 reads GEMINI_API_KEY_2 directly from env in the orchestrator
        (intentional — keeps Phase 1 ↔ Phase 2 quotas independent)."""
        import inspect
        from orchestrator import Orchestrator

        # Find the _phase2_gemini_adapter method
        src = inspect.getsource(Orchestrator._phase2_gemini_adapter)
        assert 'GEMINI_API_KEY_2' in src, \
            "Phase 2 must read GEMINI_API_KEY_2 (different daily quota)"
        # Falls back to GEMINI_API_KEY only if KEY_2 absent
        assert 'fallback' in src.lower() or 'or' in src, \
