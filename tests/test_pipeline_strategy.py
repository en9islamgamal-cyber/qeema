...
class TestEngineAvailability:
    def test_no_tafsir_validator_strategy_still_builds(self):
        """v22.5: strategy doesn't track Claude/heuristic anymore.
        Validator absence is handled by the orchestrator at runtime."""
        s = StrategyFactory.build(
            requested_mode=QualityMode.HIGH,
            quota_manager=MockQuotaManager(leo=100, el=20000),
            episode_number=1,
            has_tafsir_validator=False,
            has_leonardo_engine=True,
            has_multi_task_engine=True,
        )
        # Strategy builds successfully — the gate is in the orchestrator
        assert s.mode == QualityMode.HIGH
