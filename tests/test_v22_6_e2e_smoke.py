...
# ════════════════════════════════════════════════════════════════
# sys.modules injection helper — works whether google-genai is installed
# or not (CI pre_check job runs tests before pip install).
# ════════════════════════════════════════════════════════════════
@contextlib.contextmanager
def fake_google_genai_in_sys_modules():
    """Inject a fake `google.genai` into sys.modules for the duration of
    the with-block. Restores prior state on exit.

    Why not patch.dict("sys.modules", ...) + patch("google.genai.types")?
    Because patch("google.genai.types") *imports* google.genai under the
    hood to resolve the target — which fails in CI pre_check where
    google-genai isn't installed yet. Direct sys.modules manipulation
    bypasses that.
    """
    ...
    fake_genai = MagicMock(name="google_genai_module")
    ...
    google_module = saved_google
    if google_module is None:
        google_module = MagicMock(name="google_module")
...
class TestE2EBatchEngines:
    ...
    def test_script_engine_with_mocked_gemini(self):
        ...
        with fake_google_genai_in_sys_modules():
