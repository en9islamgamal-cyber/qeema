...
• Strategy: Holistic Prompting (طلب السكريبت كاملاً لتوفير الكوتة)
• Persona: الشيخ الأزهري "الجد أبو زياد"
• Visuals: Pixar 3D Animation Prompts
═══════════════════════════════════════════════════════
"""
...
{{
  ...
  "ayah_scenes": [ {{"ayah_number": int, "intro_text": "string", "explain_text": "string", "visual_prompt": "string"}} ],
  "outro_scene": {{"narrator_text": "string", "visual_prompt": "string"}}
    ...
    def _build_script(self, ep_num, info, data, verified):
        ...
        return EpisodeScript(
            ...
            intro_scene=NarratorScene(scene_id=1, scene_type=SceneType.INTRO, duration_sec=25, 
                                     narrator_text=data["intro_scene"]["narrator_text"], visual_prompt=data["intro_scene"]["visual_prompt"], mood=AudioMood.INTRO),
            ayah_scenes=ayah_scenes, mid_scenes=[],
            outro_scene=NarratorScene(scene_id=99, scene_type=SceneType.OUTRO, duration_sec=25,
                                     narrator_text=data["outro_scene"]["narrator_text"], visual_prompt=data["outro_scene"]["visual_prompt"], mood=AudioMood.OUTRO)
        )
