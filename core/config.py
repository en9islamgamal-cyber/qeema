"""
core/config.py — VALUE / QEEMA v15.0
=====================================================
[Changes v15]
- AudioConfig: updated ElevenLabs defaults (stability 0.68, similarity 0.88,
  style 0.30, new speed 0.85); Google TTS speaking_rate 0.85
- EngineConfig: add_ssml=True, enable_prompt_crafting=True (both were False)
- BrandingConfig: intro_duration 5→3.5s, outro_duration 5→7.0s
- ProceduralConfig: particle_count 80→35 (less visual distraction)
- AudioConfig: quran_reciter configurable (was hard-coded in voice_engine)
"""
...
@dataclass(frozen=True)
class AudioConfig:
    # ElevenLabs voice settings — v15 tuned for "جدو أبو زياد" grandfather warmth
    elevenlabs_model: str = "eleven_multilingual_v2"
    elevenlabs_stability: float = 0.68      # v15: 0.50→0.68 — steadier, less wobbly
    elevenlabs_similarity: float = 0.88     # v15: 0.85→0.88 — tighter voice clone
    elevenlabs_style: float = 0.30          # v15: 0.50→0.30 — calmer, not theatrical
    elevenlabs_speed: float = 0.85          # v15: NEW — 15% slower for ages 5-8
    elevenlabs_speaker_boost: bool = True
    # Google TTS fallback
    google_voice: str = "ar-XA-Wavenet-B"
    google_speaking_rate: float = 0.85      # v15: 0.95→0.85 — slower for children
    google_pitch: float = -1.0
    # Mastering targets (EBU R128 compliant)
    target_lufs: float = -16.0
    true_peak: float = -1.5
    # Quran audio handling
    quran_start_delay_ms: int = 600
    quran_fade_in_sec: float = 1.0
    quran_end_padding_sec: float = 1.5
    quran_reciter: str = "alafasy"          # v15: configurable (was hard-coded)
...
class BrandingConfig:
    ...
    intro_duration_sec: float = 3.5         # v15: 5.0→3.5 — viewers skip long intros
    outro_duration_sec: float = 7.0         # v15: 5.0→7.0 — more time to subscribe


@dataclass(frozen=True)
class ProceduralConfig:
    palettes: Dict[str, List[str]] = field(default_factory=lambda: dict(PALETTES))
    particle_count: int = 35                # v15: 80→35 — less visual distraction
...
class EngineConfig:
    ...
    # v15: BOTH enabled (were False by mistake/oversight)
    enable_prompt_crafting: bool = True     # v15: False→True
    prompt_crafting_model: str = "gemini-2.5-flash"

    # v15: SSML enabled for natural Arabic pauses
    add_ssml: bool = True                   # v15: False→True

    # Cinematic pipeline feature flags
