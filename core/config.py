"""
core/config.py — VALUE / QEEMA v22.5 — application configuration (env loading)
=====================================================
[v19 changes]
...
class APIKeysConfig:
    ...
    # ─── v22.5: Role-specific Gemini key splits ──────────────────
    @property
    ...
    def phase2_gemini_key(self) -> str:
        """Gemini key for Phase 2 work (deep visuals + TTS director).

        With 3 keys: returns key 2.
        With 2 keys: returns key 2.
        With 1 key:  shares with Phase 1 (returns key 1).
        With 0 keys: returns "".
        """
        ...
        return self.gemini_keys[1]

    @property
    def tafsir_review_key(self) -> str:
        """Gemini key dedicated to tafsir religious validation.

        v22.5 FINAL: Tafsir validation is part of Phase 1 (it gates the script).
        We co-locate it on key #1 since both happen on Day 1, and 14 total
        calls is trivial under 5 RPM.

        With 3+ keys: returns key 1 (same as script pool).
        With 1 key:  returns the only key.
        With 0 keys: returns "".
        """
        if not self.gemini_keys:
            return ""
    ...
    def validate(self) -> List[str]:
        ...
        if not self.gemini_keys:
            missing.append("GEMINI_API_KEY (Groq removed in v22.5.6 — Gemini-only)")
...
class PathsConfig:
    ...
    def from_root(cls, root: Path) -> "PathsConfig":
        ...
        return cls(
            ...
            amiri_font=assets / "Amiri-Bold.ttf",
...
@dataclass(frozen=True)
class VideoConfig:
    # v22.5: 720p (was 1080p). The user's reference NotebookLM video is 1280×720.
    # Reasoning:
    #   - User explicitly chose 720p ("720 كويسه مش لازم 1080")
    #   - Static-image children's content doesn't benefit from 4K-grade resolution
    #   - 720p reduces render time ~55% and file size ~50% vs 1080p
    #   - YouTube auto-upscales the playback for higher-bandwidth viewers
    #   - Leonardo Lightning XL outputs ~1024×576 already — 720p is nearer to native
    width: int = 1280
    height: int = 720
    # v22.5: fps=30 (was 60). The reference video is 24fps cinematic.
    # For static-image kid's content, 60fps is pure waste:
    # - 2× FFmpeg encoding time per episode
    # - 2× output file size with no visual benefit (images don't move at 60Hz)
    # - GitHub Actions runner minutes 2× higher
    # 30fps is the sweet spot: standard for YouTube, smooth for transitions/zooms,
    # half the render cost. Use VIDEO_FPS env var to override if needed.
    fps: int = 30
    codec: str = "libx264"
    profile: str = "high"
    crf: int = 17
...
class ImageGenConfig:
    ...
    # v22.5: 1280×720 to match the final 720p video resolution exactly.
    # Previously 1536×864 forced a downscale during render (extra FFmpeg work
    # + potential softening). Native-resolution images render crisper.
    # Leonardo accepts any dim divisible by 8 up to 1536, so 1280×720 is safe.
    width: int = 1280
    height: int = 720
    # Quality settings
