...
class APIKeysConfig:
    ...
    def script_pool_keys(self) -> Tuple[str, ...]:
        """Gemini keys reserved for the script generation pool.

        Strategy: use first N-1 keys for the script pool, leaving the last
        key dedicated to tafsir validation. With 3 keys total: keys 1-2 →
        script, key 3 → tafsir review.

        With fewer keys, falls back gracefully:
            1 key  → all script (no dedicated tafsir; uses the same key)
            2 keys → key 1 script, key 2 tafsir
            3+ keys → all but last for script
        """
    ...
    def tafsir_review_key(self) -> str:
        """Dedicated Gemini key for tafsir religious validation.

        With 3 keys: returns key 3 (GEMINI_API_KEY_3).
        With 2 keys: returns key 2.
        With 1 key:  returns the same key (shared with script).
        With 0 keys: returns empty string.
        """
