"""
core/resilience.py — VALUE / QEEMA v22.5 — CircuitBreaker + ProviderPool + RetryPolicy
=========================================================
    ...
    def execute(
        ...
        *,
        ...
        for _ in range(attempts):
            ...
            # Rate limit gate
            # v22.5: Increased timeout from 5s → 60s. Gemini free tier
            # is 5 RPM (1 call per 12s). With short timeout, the pool would
            # skip every provider after the first burst, then fail with
            # "all providers unavailable" instead of waiting for the bucket
            # to refill. 60s lets the bucket refill ~5x.
            if provider.rate_limiter and not provider.rate_limiter.acquire(timeout_sec=60.0):
                logger.warning(
