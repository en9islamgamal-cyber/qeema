"""
core/tafsir_cache.py — VALUE / QEEMA v22.5
=========================================================================
...
class CachedTafsirFetcher:
    ...
    def fetch_combined(self, surah: int, ayah: int) -> Optional[str]:
        """Cached version of fetch_combined.

        Mirrors AuthenticTafsirFetcher.fetch_combined exactly: pulls As-Saadi (#169)
        + Al-Muyassar (#16) and concatenates them with the same labels. Each
        underlying fetch goes through this CachedTafsirFetcher's per-tafsir cache,
        so on a hit there is ZERO network traffic. On a miss it delegates to the
        wrapped fetcher's individual fetch() and persists the result.
        """
        # Tafsir source IDs — must match the constants in tafsir_validator.py.
        # Kept inline (not imported) to avoid a circular core ↔ engines import.
        TAFSIR_SAADI = 169
        TAFSIR_MUYASSAR = 16
