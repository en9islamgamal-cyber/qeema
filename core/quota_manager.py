"""
core/quota_manager.py — VALUE / QEEMA v22.5 — monthly quota tracker (ElevenLabs + Leonardo)
=========================================================================
...
class QuotaConfig:
    ...
    # CAMB.AI fallback monthly budget. v22.5: hard cap to prevent runaway
    # spending if ElevenLabs goes down for an extended period. Default = 50k
    # chars (≈ 7 episodes × 7000 chars each — enough to cover one full month
    # of episodes if EL is completely down, but not more). Adjust via
    # CAMB_AI_MONTHLY_CHARS env var or QuotaConfig override.
    camb_monthly_chars: int = 50_000
...
class QuotaState:
    ...
    # v22.5: CambAI is a fallback — track separately so a runaway fallback
    # path can be detected and capped without affecting ElevenLabs metrics.
    camb_used_this_month: int = 0
...
class QuotaManager:
    ...
    def _maybe_reset_monthly(self) -> None:
        ...
        if self._state.month_key != current:
            ...
            self._state.camb_used_this_month = 0  # v22.5: reset fallback counter
    ...
    # ─── CambAI (TTS fallback) ──────────────────────────────────
    def camb_remaining(self) -> int:
        """Chars remaining in CambAI monthly budget."""
        return max(0,
            self._config.camb_monthly_chars
            - self._state.camb_used_this_month
        )

    def can_consume_camb(self, estimated_chars: int) -> bool:
        """Check if CambAI fallback has budget for this call.

        v22.5: CambAI is a charged service used as fallback. To prevent
        runaway spending if ElevenLabs goes down for an extended period,
        we hard-cap monthly CambAI usage. When the cap is hit, the fallback
        chain falls through to Google TTS instead.
        """
        if estimated_chars <= 0:
            return True
        return estimated_chars <= self.camb_remaining()

    def consume_camb(self, actual_chars: int) -> None:
        """Record actual CambAI usage (after successful call)."""
        self._state.camb_used_this_month += max(0, actual_chars)
        self._save_state()
    ...
    def report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "camb_ai": {
                "used": self._state.camb_used_this_month,
                "total": self._config.camb_monthly_chars,
                "remaining": self.camb_remaining(),
                "utilization_percent": round(
                    self._state.camb_used_this_month
                    / max(self._config.camb_monthly_chars, 1) * 100, 1
                ),
            },
        }
    ...
    def print_report(self) -> None:
        ...
        camb = r['camb_ai']
        print(f"CambAI:     {camb['used']:,}/{camb['total']:,} chars "
              f"({camb['utilization_percent']}% — {camb['remaining']:,} remaining)\n")
