    ...
    def synthesize(self, text: str, output_path: str) -> TTSResult:
        ...
        if self._quota_manager is not None:
            if not self._quota_manager.can_consume_elevenlabs(char_estimate):
                # Try Google TTS fallback if available
                if "google_tts" in self._providers:
                    logger.warning(
                        f"⚠️ ElevenLabs quota low — falling back to Google TTS for {char_estimate} chars"
                    )
                    request = TTSRequest(text=normalized, output_path=output_path)
                    result = self._providers["google_tts"].synthesize(request)
                    return result
                else:
                    raise AudioGenerationError(
                        f"ElevenLabs quota exhausted ({self._quota_manager.elevenlabs_remaining()} remaining), "
        ...
        # v19: Record actual consumption (only if ElevenLabs was used)
        if self._quota_manager is not None and result.provider == "elevenlabs":
