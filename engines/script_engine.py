"""
engines/script_engine.py — VALUE / QEEMA v22.5 — legacy single-task script engine + ProviderPool
==================================================================================
...
═══ قواعد دقة دينية إلزامية (v22.6) ═══

كل تشبيه أو شرح لازم يحترم القواعد دي بالدقة:

1. **الآيات اللي بتتكلم عن "اليوم الآخر" أو "يوم الحساب"** (مثل
   "مالك يوم الدين"، "إذا زلزلت الأرض"):
   ❌ ممنوع تماماً تشبيهها بالعمليات البيولوجية الدائمة (دقات القلب،
      الخلايا، التنفس). اليوم الآخر يوم محدد بيحصل، مش حالة دائمة في الجسم.

2. **آيات العبادة والاستعانة** (مثل "إياك نعبد وإياك نستعين"):
   ❌ ممنوع تماماً تشبيهها بقوى ميكانيكية (المغناطيس، الجاذبية،
      الانجذاب الفيزيائي). العبادة اختيار حر بمحبة، مش انجذاب آلي.

3. **آيات "غضب الله" أو "الضالين"** (مثل "غير المغضوب عليهم ولا الضالين"):
   ❌ ممنوع تماماً تشبيهها بأكل صحي وأكل غير صحي، أو أي تشبيه طعامي.
      المعنى أعمق من ذلك بكثير ويتعلق بالبعد عن طريق الله.

4. **"بسم الله الرحمن الرحيم"**:
   ❌ ممنوع تماماً وصفها بـ "كود سري"، "كلمة سحرية"، أو أي شيء يوحي
      بالسحر. هي البركة والاستعانة بالله في كل أمر.

✅ التشبيهات المسموحة لكل ده: من الطبيعة (البذرة تنبت، الشمس تشرق،
   الورد يتفتح)، من الحياة اليومية (الخريطة، البيت، الكتاب)، من
   الأخلاق (الأم تحب ولدها، الأب يرعى أولاده).
...
_ANALOGY_DOMAINS: List[str] = [
    ...
    "العلوم البسيطة (الذرة، الموجات، الحرارة، الضوء)",
...
class ScriptEngine:
    ...
    def __init__(
        ...
        *,
        ...
        if tafsir_validator:
            logger.info("✅ Religious validator wired (Gemini reviewer)")
        if hook_optimizer:
