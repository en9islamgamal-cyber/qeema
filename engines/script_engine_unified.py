"""
engines/script_engine_unified.py — VALUE / QEEMA v22 (NEW)
=========================================================================
...
class UnifiedScriptEngine:
    ...
    def _generate_multi_task(self, episode_number: int) -> EpisodeScript:
        ...
        # 5d. v22.5: TTS Director — per-segment voice direction with SSML
        # Adds <break> tags + pronunciation hints to text fields. Uses one
        # of the script_pool adapters (gemini-1) since the script call already
        # finished. The result is stored on the validated dict so Phase 2
        # (audio gen) picks it up.
        try:
            adapter = self._get_first_adapter()
            if adapter is not None:
                ...
                if episode_direction.directions:
                    ...
                    logger.info(
                        f"🎙️ TTS Director applied to "
                        f"{len(episode_direction.directions)} segments"
                    )
                else:
                    logger.info(
                        "ℹ️ TTS Director produced no directions (using base voice settings)"
                    )
        except Exception as e:
            logger.warning(
                f"⚠️ TTS Director skipped ({e.__class__.__name__}: {e}) — "
                "voice will use base emotion settings only"
            )

        # 5e. v22.5: Deep visual prompts — chained Gemini calls for richer Leonardo prompts
        # 21 calls total (3 layers × 7 scenes). Result stored on episode dict.
        # Phase 2 (image gen) reads validated["_deep_visuals"] and uses
        # VisualPromptEngineer.build_from_deep_result() to assemble the Leonardo prompt.
        try:
            adapter = self._get_first_adapter()
            if adapter is not None:
                ...
                deep_results = deep_gen.generate_for_episode(
                    validated.get("ayah_scenes", []),
                    max_workers=3,
                )
                # Store as plain dicts so it survives JSON serialization
                validated["_deep_visuals"] = [
                    {
                        "subject": r.subject,
                        "action": r.action,
                ...
                fully_complete = sum(1 for r in deep_results if r.is_complete)
                logger.info(
                    f"🎨 Deep visual prompts: {successful}/{len(deep_results)} usable, "
                    f"{fully_complete} fully completed (3 layers)"
                )
        except Exception as e:
            logger.warning(
                f"⚠️ Deep visual prompts skipped ({e.__class__.__name__}: {e}) — "
