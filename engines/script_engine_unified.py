...
class UnifiedScriptEngine:
    ...
    def generate(
        ...
        *,
        ...
        v22.6 ARCHITECTURE:
          1. Try BatchScriptEngine on Key 1 (1 Gemini call for all 7 ayahs)
             - Uses Pydantic response_schema for guaranteed valid JSON
             - Strict religious accuracy rules in prompt
          2. If batch fails → fall back to legacy per-ayah path (7 calls)
          3. If legacy fails → raise (caller's problem)

        Quota: 1 call (batch success) or 7 calls (legacy fallback).
        Both well within 20/day budget for Key 1.
        """
        # Try batch path first (v22.6)
        try:
            batch_script = self._try_batch_script(episode_number)
            if batch_script is not None:
                logger.info("✅ Script generation via batch (1 Gemini call)")
                return batch_script
        except Exception as e:
            logger.warning(
                f"⚠️ Batch script failed ({type(e).__name__}: {e}) — "
                f"falling back to legacy per-ayah path"
            )

        # Fallback: legacy per-ayah (proven reliable, 7 calls)
        logger.info("📝 Script generation: legacy per-ayah path (7 LLM calls)")
        return self._legacy.generate(episode_number)

    def _try_batch_script(
        self, episode_number: int,
    ) -> Optional[EpisodeScript]:
        """Try to generate via BatchScriptEngine on Key 1.

        Returns None on failure (caller falls back to legacy).
        """
        # Get Key 1's Gemini client
        adapters = getattr(self._legacy, "_adapters", {})
        if "gemini-1" not in adapters:
            return None
        ...
        # Look up episode info + ayahs + tafsirs
        ...
        # Fetch tafsirs (best-effort)
        tafsirs: Dict[int, str] = {}
        try:
            from engines.tafsir_validator import AuthenticTafsirFetcher
            fetcher = AuthenticTafsirFetcher()
            for a in ayahs:
                try:
                    tafsirs[a["number"]] = fetcher.fetch_combined(
                        info["surah"], a["number"]
                    ) or ""
                except Exception:
                    tafsirs[a["number"]] = ""
        except Exception as e:
            logger.warning(f"⚠️ Tafsir fetcher unavailable: {e}")

        # Build batch engine on Key 1's client
        from engines.batch_engines import BatchScriptEngine
        # v22.6.1: pass full adapters dict for multi-key rotation
        engine = BatchScriptEngine(adapters)
        ...
        # Convert BatchScriptOut → EpisodeScript
        script = self._batch_to_episode_script(
            episode_number, info, ayahs, result,
        )

        # Save to disk (legacy engine handles atomic save)
        if hasattr(self._legacy, "_save_atomic"):
            try:
                self._legacy._save_atomic(episode_number, script)
                logger.info(f"💾 Batch script cached to disk")
            except Exception as e:
                logger.warning(f"⚠️ Disk save failed: {e}")

        return script

    @staticmethod
    def _batch_to_episode_script(
        episode_number: int,
        info: Dict[str, Any],
        ayahs: List[Dict[str, Any]],
        batch: Any,  # BatchScriptOut
    ) -> EpisodeScript:
        """Convert BatchScriptOut → EpisodeScript Pydantic model."""
        intro_scene = NarratorScene(
            scene_id=0,
            scene_type=SceneType.INTRO,
            narrator_text=batch.intro_text,
            visual_prompt="",
            visual_scene=VisualScene.GOLDEN_FIELD,
        ...
        outro_scene = NarratorScene(
            ...
            narrator_text=batch.outro_text,
            visual_prompt="",
            visual_scene=VisualScene.STARRY_NIGHT,
            palette=PaletteName.NIGHT_STARS,
            mood=AudioMood.REVERENT,
            keywords=[],
        )

        # Match each batch ayah with its verified ayah by number
        ayah_scenes: List[AyahScene] = []
        ayahs_by_num = {a["number"]: a for a in ayahs}

        for i, batch_ayah in enumerate(batch.ayahs, 1):
            ayah_dict = ayahs_by_num.get(batch_ayah.ayah_number)
            if not ayah_dict:
                continue

            ayah_obj = VerifiedAyah(
                surah=ayah_dict.get("surah", info["surah"]),
                number=ayah_dict["number"],
                text=ayah_dict["text"],
                audio_url=ayah_dict.get("audio_url"),
            )

            # Map emotion string → enum
            try:
                emotion = SceneEmotion(batch_ayah.scene_emotion.lower())
            except ValueError:
            ...
            scene = AyahScene(
                ...
                ayah=ayah_obj,
                intro_text="",
                hook_text=batch_ayah.hook_text,
                story_text=batch_ayah.story_text,
                explain_text=batch_ayah.explain_text,
                moral_text=batch_ayah.moral_text,
                narrator_text=batch_ayah.explain_text,
                visual_scene=VisualScene.GARDEN,
                palette=PaletteName.GOLDEN_HOUR,
                visual_prompt="",  # Phase 2 fills this
                scene_emotion=emotion,
                transition_type=TransitionType.FADE,
                keywords=[],
                mood=AudioMood.CALM,
            )
            ayah_scenes.append(scene)

        return EpisodeScript(
            episode_number=episode_number,
            surah_name=info["name"],
            title=batch.title,
            youtube_title=batch.youtube_title,
            youtube_description=batch.youtube_description,
            youtube_tags=[],
            cta_text="",
