...
# ════════════════════════════════════════════════════════════════
# v22.6: Deterministic Forbidden-Analogy Detector
# ════════════════════════════════════════════════════════════════
class ForbiddenAnalogyDetector:
    """Defense-in-depth keyword-level check for the four canonical
    doctrinal errors that the Gemini reviewer is also asked to catch.

    Why both? An LLM reviewer can occasionally miss a flagrant analogy when
    the surrounding tafsir is silent on it (e.g., the source tafsir doesn't
    discuss magnets, so the reviewer "doesn't see" the magnet analogy as
    contradicting anything). This deterministic check catches the keyword
    pattern even when the LLM blinks.

    The detector is INTENTIONALLY conservative: it raises the concern but
    does NOT downgrade `passed` automatically — that's the orchestrator's
    decision (so the reviewer's confidence can override on a true positive
    in edge cases like discussing the analogies academically). It just
    surfaces the concern so it appears in the validation report.

    NOTE on Arabic morphology: the patterns use word stems and ignore
    diacritics. They match common dialect forms (مغناطيس, مغناطيسية,
    المغناطيس, etc).
    """

    # Each rule: (rule_id, ayah_topic_keywords, forbidden_keywords, description)
    # `ayah_topic_keywords` — phrases in the ayah text or scope that
    #   indicate this rule applies (so we don't flag academic mentions
    #   of magnets when the ayah is about gardens).
    # `forbidden_keywords` — patterns whose presence in the analogy
    #   triggers the concern.
    _RULES: Tuple[Tuple[str, Tuple[str, ...], Tuple[str, ...], str], ...] = (
        (
            "judgment-day-as-biology",
            ("يوم الدين", "يوم الحساب", "اليوم الآخر", "البعث", "زلزلت"),
            ("نبض", "خلية", "خلايا", "تنفس", "تنفّس", "عضلات",
             "biological", "cellular"),
            "تشبيه يوم الحساب بعمليات الجسم البيولوجية الدائمة "
            "(red flag #1: اليوم الآخر يوم محدد، مش حالة دائمة)",
        ),
        (
            "worship-as-magnet",
            ("إياك نعبد", "نستعين", "العبادة", "اعبد"),
            ("مغناطيس", "ممغنط", "جاذبية الأرض", "انجذاب فيزيائي",
             "الانجذاب الفيزيائي"),
            "تشبيه العبادة بقوى ميكانيكية "
            "(red flag #2: العبادة اختيار حر بمحبة، مش انجذاب آلي)",
        ),
        (
            "wrath-as-food",
            ("المغضوب", "الضالين", "غضب الله", "ضلال"),
            ("أكل صحي", "أكل غير صحي", "الطعام الصحي", "أكل غير سليم",
             "junk food"),
            "تشبيه الغضب/الضلال بالأكل الصحي وغير الصحي "
            "(red flag #3: المعنى أعمق من ذلك بكثير)",
        ),
        (
            "basmala-as-magic",
            ("بسم الله", "البسملة"),
            ("كود سري", "كود سحري", "كلمة سحرية", "تعويذة",
             "secret code", "magic word", "spell"),
            "وصف بسم الله بـ كود سري أو كلمة سحرية "
            "(red flag #4: هي البركة والاستعانة، مش سحر)",
        ),
    )
    ...
    def check(
        ...
        *,
        ...
        """Scan an ayah explanation for forbidden-analogy patterns.

        Returns:
            A list of human-readable Arabic concerns. Empty list if all clear.
            Each concern names the rule that fired so reviewers can find it.
        """
        # Normalize for matching — strip Arabic diacritics, fold whitespace.
        ayah_norm = cls._normalize(ayah_text)
        body_norm = cls._normalize(f"{explanation}\n{analogy}")

        concerns: List[str] = []
        for rule_id, topic_kws, forbidden_kws, description in cls._RULES:
            ayah_matches = any(
                cls._normalize(kw) in ayah_norm for kw in topic_kws
            )
            if not ayah_matches:
                continue  # rule's topic not in this ayah — skip

            forbidden_hit = next(
                (kw for kw in forbidden_kws
                 if cls._normalize(kw) in body_norm),
                None,
            )
            if forbidden_hit:
                concerns.append(
                    f"[{rule_id}] {description} "
                    f"(الكلمة المرصودة: «{forbidden_hit}»)"
                )
    ...
    def _normalize(s: str) -> str:
        """Strip Arabic diacritics + normalize alif wasla + fold whitespace + casefold.

        Quranic text from quran.com API uses U+0671 (ٱ alif wasla) wherever
        Modern Standard Arabic would use U+0627 (ا). Our canonical
        topic-keywords are written in MSA, so without this normalization NO
        topic match would ever fire on real API-fetched ayah text. This is
        the most critical line in the detector.

        We do NOT collapse other alif variants (إ/أ/ا) — different forms have
        different morphological meanings, and our forbidden_keywords use
        ...
        s = re.sub(r"[\u064B-\u065F\u0670]", "", s)
        # Normalize alif wasla (ٱ U+0671) to plain alif (ا U+0627).
        # Quranic mushafs use U+0671; MSA uses U+0627.
        s = s.replace("\u0671", "\u0627")
        # Collapse whitespace
...
class GeminiReviewer:
    ...
    def _build_review_prompt(
        ...
        v22.6: Adds an explicit list of forbidden-analogy red flags. The
        reviewer is now actively looking for the four canonical doctrinal
        errors, instead of inferring them from authentic tafsir comparison
        alone (which was passing them through when the source tafsir was
        silent on the analogy).
        """
...
افحص الشرح والتشبيه ضد القائمة دي. لو لقيت أي واحد منهم → passed=false:
...
2. **آيات العبادة والاستعانة** (مثل: إياك نعبد وإياك نستعين):
   ❌ ممنوع تشبيهها بقوى ميكانيكية (المغناطيس، الجاذبية، الانجذاب
      الفيزيائي). العبادة اختيار حر بمحبة، مش انجذاب آلي.

3. **آيات الغضب والضلال** (مثل: غير المغضوب عليهم ولا الضالين):
   ❌ ممنوع تشبيهها بالأكل الصحي مقابل الأكل غير الصحي، أو أي تشبيه
      طعامي. المعنى أعمق من ذلك بكثير.

4. **"بسم الله الرحمن الرحيم"**:
   ❌ ممنوع وصفها بـ "كود سري"، "كلمة سحرية"، أو أي شيء يوحي بالسحر.

[مهمتك]
قارن الشرح المقترح بالتفسير الأصيل. حدد:

1. **passed (boolean):**
   - true: الشرح صحيح، متوافق مع التفسير، ومايقعش في أي red flag
   - false: فيه خطأ، انحراف عن المعنى، أو وقع في red flag

2. **confidence (0.0-1.0):** ثقتك في حكمك
   - 0.9+: الحكم قاطع وواضح (red flag صريح، أو شرح متطابق)
...
3. **concerns (list of strings):** المشاكل الفعلية المحددة
   - أي خطأ عقدي (إذكره صريحاً، حدد أي red flag وقع فيه)
...
- ✋ التشبيهات ممكنة لو ما خالفتش المعنى ولا الـ red flags
...
3. **concerns (list of strings):** المشاكل الفعلية المحددة لو وُجدت
   (إذكر صراحة لو وقع في أي red flag وأي رقم منه)
...
class TafsirValidator:
    ...
    def validate_explanation(
        ...
        # v22.6 Layer 1.5: Deterministic forbidden-analogy keyword check.
        # Runs BEFORE Gemini (cheap, no quota cost). If it fires, the result
        # is forced to passed=False regardless of what Gemini says — these
        # are the four canonical doctrinal red flags.
        deterministic_concerns = ForbiddenAnalogyDetector.check(
            ayah_text=ayah_text,
            explanation=llm_explanation,
            analogy=llm_analogy or "",
        )
        if deterministic_concerns:
            logger.warning(
                f"🚫 ForbiddenAnalogyDetector fired on {surah_name} "
                f"ayah {ayah_number}: {len(deterministic_concerns)} hit(s)"
            )
            for c in deterministic_concerns:
                logger.warning(f"   └─ {c}")
        ...
        if result.confidence < self._confidence_threshold:
            return TafsirValidationResult(
                ...
                concerns=result.concerns + deterministic_concerns + [
        ...
        # v22.6: Even if Gemini said "pass", a deterministic red flag
        # forces failure. The detector is conservative and only fires on
        # clear keyword pattern matches in the right ayah context.
        if deterministic_concerns:
            return TafsirValidationResult(
                passed=False,
                confidence=max(result.confidence, 0.95),  # high — keyword match is unambiguous
                concerns=result.concerns + deterministic_concerns,
                authentic_excerpt=result.authentic_excerpt,
                reviewer=f"{result.reviewer}+forbidden-detector",
            )
